...
-- Source: TransactionHistory.csv (UTF-8, tab-delimited, no header row)

CREATE OR REPLACE TABLE production.transaction_history AS
SELECT
...
FROM read_csv(
    '{folder_path}/TransactionHistory.csv',
    header = false,
    delim = '\t'
);
